...
export function getStoredPoints(): Point[] {
  try {
...
export function parsePointsFromJson(jsonStr: string): Point[] {
  ...
  const cleaned = parsed
    ...
    .filter((it: any) => !isNaN(it.lat) && !isNaN(it.lng))
  return cleaned
}
...
function App(): React.ReactElement {
  ...
  const [heatMetric, setHeatMetric] = useState<string>(() => {
    try {
      return localStorage.getItem('heat_metric') || 'download'
    } catch {
      return 'download'
    }
  })
  ...
  if (!mapRef.current) {
      ...
      ;(mapRef.current as any)._legendDiv = legend.getContainer()
    }
  ...
  const metricValues = points.map(p => (p && (p as unknown as any)[heatMetric]) || 0).filter(v => !isNaN(v))
    ...
    const legendDiv = mapRef.current && (mapRef.current as any)._legendDiv
    if (legendDiv) {
  ...
  const runSpeedTest = () => {
    ...
    // Start the speedtest
    speedtestRef.current = new (SpeedTest as any)({
      autostart: true,
      measurements: [
    ...
    speedtestRef.current.onFinish = (results: any) => {
      ...
      const newPoints = points.map(pt => {
        if (pt.lat === (currentLocation as any)?.lat && pt.lng === (currentLocation as any)?.lng) {
          updated = true
